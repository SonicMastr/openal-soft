...
// these AL threads almost exactly mimic C11 threads
// so why not just use C11 threads?
...
typedef thrd_t althrd_t;
typedef mtx_t almtx_t;
typedef cnd_t alcnd_t;
typedef int alsem_t; // no semaphores in c11 threads, they're also unused in the switch port
typedef tss_t altss_t;
typedef once_flag alonce_flag;

#define AL_ONCE_FLAG_INIT ONCE_FLAG_INIT
...
{
    return thrd_sleep(ts, rem) == thrd_success ? althrd_success : althrd_error;
}


inline int almtx_lock(almtx_t *mtx)
{
    return mtx_lock(mtx) == thrd_success ? althrd_success : althrd_error;
}

inline int almtx_unlock(almtx_t *mtx)
{
    return mtx_unlock(mtx) == thrd_success ? althrd_success : althrd_error;
}
...
// we don't actually have C11 TSS, hack time

#define HACK_TSS_MAX 32

extern __thread void *hack_tss[HACK_TSS_MAX];

inline void *altss_get(altss_t tss_id)
{
    if (tss_id >= HACK_TSS_MAX)
        return NULL;
    return hack_tss[tss_id];
