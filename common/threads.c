...
static int althrd_starter(void *arg)
{
    thread_cntr cntr;
    memcpy(&cntr, arg, sizeof(cntr));
    free(arg);

    return (int)((*cntr.func)(cntr.arg));
}

// actually modify libnx's thrd_start to increase stack size
...
static int hack_thrd_create(thrd_t *thr, thrd_start_t func, void *arg)
{
    if (!thr || !func)
        return thrd_error;
...
int althrd_create(althrd_t *thr, althrd_start_t func, void *arg)
{
    ...
    cntr->func = func;
    cntr->arg = arg;
    if((err=hack_thrd_create(thr, althrd_starter, cntr)) == thrd_success)
        return althrd_success;
...
int althrd_detach(althrd_t thr)
{
    // thrd_detach is unavailable in libnx
    return althrd_success;
}
...
int almtx_init(almtx_t *mtx, int type)
{
    ...
    if(type == almtx_plain)
        ...
        ret = mtx_init(mtx, mtx_recursive);
    return ret != thrd_success ? althrd_error : althrd_success;
}
...
int alcnd_wait(alcnd_t *cond, almtx_t *mtx)
{
    if(cnd_wait(cond, mtx) == thrd_success)
        return althrd_success;
    return althrd_error;
}
...
__thread void *hack_tss[HACK_TSS_MAX] = { NULL };
static __thread int hack_tss_taken[HACK_TSS_MAX] = { 0 };

static inline u32 hack_tss_find_key(void)
{
    for (int i = 0; i < HACK_TSS_MAX; ++i)
        if (!hack_tss_taken[i])
            return i;
    return HACK_TSS_MAX;
}
...
int altss_create(altss_t *tss_id, altss_dtor_t callback)
{
    u32 key = hack_tss_find_key();
    if (key >= HACK_TSS_MAX)
        return althrd_error;
    *tss_id = key;
    hack_tss[key] = NULL;
    hack_tss_taken[key] = 1;
    // the only thing that uses these in any significant way on the switch port is
    // is alc_init(), which sets the dtor to a function that releases the thread context
    // while that would basically leak memory, I don't particularly care
    (void)callback;
    return althrd_success;
}
...
void altss_delete(altss_t tss_id)
{
    if (tss_id >= HACK_TSS_MAX)
        return;
    hack_tss[tss_id] = NULL;
    hack_tss_taken[tss_id] = 0;
}
